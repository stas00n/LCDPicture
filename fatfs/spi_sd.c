...
void spi_init(void)
{
	...
	GPIO_PinAFConfig(GPIOB, GPIO_PinSource13, GPIO_AF_SPI2);  	// INIT SCK
	GPIO_PinAFConfig(GPIOB, GPIO_PinSource14, GPIO_AF_SPI2);	// INIT MISO
	GPIO_PinAFConfig(GPIOB, GPIO_PinSource15, GPIO_AF_SPI2);	// INIT MOSI
	...
	// configure SPI1
	SPI_Cmd(SPI2, DISABLE); // Disable SPI1....
	SPI_I2S_DeInit(SPI2);
...
)
{
	...
	case CTRL_ERASE_SECTOR :	/* Erase a block of sectors (used when _USE_ERASE == 1) */
		...
		dp = (uint32_t*) buff; st = dp[0]; ed = dp[1];				/* Load sector block */
